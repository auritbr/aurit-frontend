...
async function responseError(response: Response) {
  ...
  try {
    ...
    return String(
      body.message ?? body.mensagem ?? body.error ?? body.detail ?? text,
    );
  } catch {
    return text;
  }
  ...
  if (Array.isArray(body)) {
    return body
      .map((item) =>
        typeof item === "string"
          ? item
          : String((item as Record<string, unknown>).modulo ?? ""),
      )
      .filter(Boolean);
  }
...
export async function previewImport(
  ...
  const response = await fetch(
    `${API_URL}/importacoes/${encodeURIComponent(module)}/preview`,
    {
      method: "POST",
