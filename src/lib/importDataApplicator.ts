...
export function normalizeBrazilianState(value: unknown) {
  ...
  return (
    Object.values(BRAZILIAN_STATES_BY_UF).find(
      (state) => normalizeComparable(state) === normalized,
    ) ?? raw
  );
}

function isAddressStateField(
...
export function isImportValueEmpty(value: unknown): boolean {
  ...
  if (Array.isArray(value))
    return value.length === 0 || value.every(isImportValueEmpty);
...
function isAllowedRelationshipId(
  value: unknown,
  options: ImportFieldRule["options"],
) {
  if (options?.length)
...
function applyRule(
  ...
  if (rule.kind === "enum") {
    ...
    warnings.push({
      field,
      value,
      message: `“${String(value)}” não corresponde a uma opção disponível.`,
    });
    return undefined;
  }
    ...
    for (const item of values) {
      ...
      if (option) matched.push(option.value);
        ...
        warnings.push({
          field,
          value: item,
          message: `“${String(item)}” não corresponde a uma opção disponível.`,
        });
    }
    return matched;
  }
  if (rule.kind === "relationship") {
    ...
    warnings.push({
      field,
      value,
      message: "Relacionamento recebido como texto; selecione-o manualmente.",
    });
    return undefined;
  }
  const values = Array.isArray(value) ? value : [value];
  const ids = values
    .filter((item) => isAllowedRelationshipId(item, rule.options))
    .map(String);
  for (const item of values.filter(
    (item) => !isAllowedRelationshipId(item, rule.options),
  )) {
    warnings.push({
      field,
      value: item,
      message: `“${String(item)}” não é um ID seguro; selecione o relacionamento manualmente.`,
    });
  }
  return ids;
}
...
export function applyImportedData<T extends object>(
  ...
  for (const [sourceField, rawValue] of Object.entries(imported)) {
    ...
    if (
      normalizeComparable(sourceField) === "complemento" &&
    ...
    const targetField =
      rule?.targetField ?? (addressState ? "estado" : sourceField);
    const value = rule
    ...
    if (comparableItem(before) === comparableItem(merged))
      preservedFields.push(targetField);
