...
const sortByOptions = [
  { value: "nomeContaReceber", label: "Conta" },
  ...
  { value: "formaPagamento", label: "Forma de recebimento" },
...
export default function ContasReceber() {
  ...
  const [items, setItems] = useState<ContaReceberData[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState<ContaReceberData | null>(
    null,
  );
  const [panelOpen, setPanelOpen] = useSessionBoolean(
    "contas-receber:pesquisa-avancada",
    false,
  );
  ...
  const [pagadorNames, setPagadorNames] = useState<Record<string, string>>({});
  ...
  const pagadorLabel = (item: ContaReceberData) => {
    if (item.colaboradorId)
      return pagadorNames[`c-${item.colaboradorId}`] || "—";
    if (item.participanteId)
      return pagadorNames[`p-${item.participanteId}`] || "—";
    if (item.integranteId) return pagadorNames[`i-${item.integranteId}`] || "—";
    return item.nomePagador?.trim() || "—";
  };
  ...
  const filtered = useMemo(() => {
    const nome = normalize(filtros.nomeContaReceber);
    const pagador = normalize(filtros.pagador);
    ...
    const result = items.filter((i) => {
      if (nome && !normalize(i.nomeContaReceber).includes(nome)) return false;
      if (pagador && !normalize(pagadorLabel(i)).includes(pagador))
        return false;
      if (
        filtros.classificacao.length &&
        !filtros.classificacao.includes(i.classificacaoContaReceber)
      )
        return false;
      if (
      ...
      if (
        !inRange(
          i.dataRecebimento,
          filtros.recebimentoDe,
          filtros.recebimentoAte,
        )
      )
        return false;
      return true;
    ...
    const sortValue = (item: ContaReceberData): string | number => {
      switch (filtros.sortBy) {
        case "pagador":
          return pagadorLabel(item);
        case "classificacao":
          return classificacaoLabel(item.classificacaoContaReceber);
        case "dataCompetencia":
          return item.dataCompetencia || "";
        case "dataVencimento":
          return item.dataVencimento || "";
        case "valorAReceber":
          return parseMoney(item.valorAReceber);
        case "formaPagamento":
          return formaRecebimentoLabel(item.formaPagamento);
        case "statusFinanceiro":
          return statusFinanceiroLabel(item.statusFinanceiro);
        default:
          return item.nomeContaReceber || "";
      }
    };
  ...
  const activeFilters = useMemo<ActiveFilterItem[]>(() => {
    ...
    const pushText = (key: "nomeContaReceber" | "pagador", label: string) => {
    ...
    pushText("nomeContaReceber", "Nome da conta");
    pushText("pagador", "Pagador");
    ...
    pushMulti("formaPagamento", "Forma de recebimento", (v) =>
      labelOf(formaRecebimentoOptions, v),
    );
    ...
    pushDate("recebimentoDe", "Recebimento inicial");
    pushDate("recebimentoAte", "Recebimento final");
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteContaReceber(confirmDelete.id);
      setItems((current) =>
  ...
  const exportColumns = [
    { header: "Conta", key: "nomeContaReceber" },
    { header: "Pagador", key: "pagadorLabelText" },
    ...
    { header: "Valor a receber", key: "valorAReceberLabel" },
    { header: "Valor recebido", key: "valorRecebidoLabel" },
    { header: "Forma de recebimento", key: "formaRecebimentoLabelText" },
  ...
  const getExportData = () =>
    filtered.map((item) => ({
      ...item,
      pagadorLabelText: pagadorLabel(item),
      classificacaoLabelText: classificacaoLabel(
        item.classificacaoContaReceber,
      ),
      competenciaLabel: formatDate(item.dataCompetencia),
      vencimentoLabel: formatDate(item.dataVencimento),
      valorAReceberLabel: formatCurrency(parseMoney(item.valorAReceber)),
      valorRecebidoLabel: item.valorRecebido
        ? formatCurrency(parseMoney(item.valorRecebido))
        : "—",
      formaRecebimentoLabelText: formaRecebimentoLabel(item.formaPagamento),
  ...
  const totalAReceber = items
    ...
    .reduce((acc, i) => acc + parseMoney(i.valorAReceber), 0);
  const pendentes = items.filter(
    (i) => i.statusFinanceiro === "PENDENTE",
  ).length;
  const vencidas = items.filter((i) => i.statusFinanceiro === "VENCIDO").length;
  const recebidasItens = items.filter(
    (i) => i.statusFinanceiro === "LIQUIDADO",
  );
  const totalRecebido = recebidasItens.reduce(
    (acc, i) =>
      acc +
      (i.valorRecebido
        ? parseMoney(i.valorRecebido)
        : parseMoney(i.valorAReceber)),
    0,
  );
    ...
    <AppLayout>
      <div className="container max-w-7xl py-6 sm:py-8">
        <ListPageHeader
          ...
          objective="Cadastre e acompanhe os valores que a organização tem a receber, mantendo organizadas as informações necessárias para controlar vencimentos, recebimentos e a entrada dos recursos no financeiro da organização."
          actions={
            <Button
              type="button"
        ...
        <div className="space-y-4">
            ...
            <form onSubmit={handleSearch} noValidate>
              <SearchFilterGrid>
                <div>
                  ...
                  <Input
                    id="filtroNomeConta"
                    value={draft.nomeContaReceber}
                    onChange={(e) =>
                      setDraftField("nomeContaReceber", e.target.value)
                    }
                ...
                <div>
                  <FieldLabel htmlFor="filtroPagador">Pagador</FieldLabel>
                  <Input
                    id="filtroPagador"
                    value={draft.pagador}
                    onChange={(e) => setDraftField("pagador", e.target.value)}
                ...
                <div>
                  <FieldLabel htmlFor="filtroRecebimentoDe">
                    Recebimento inicial
                  </FieldLabel>
                  <Input
                    id="filtroRecebimentoDe"
                    type="date"
                    value={draft.recebimentoDe}
                    onChange={(e) =>
                      setDraftField("recebimentoDe", e.target.value)
                    }
                    className={dateFieldClass}
                  />
                </div>
                <div>
                  <FieldLabel htmlFor="filtroRecebimentoAte">
                    Recebimento final
                  </FieldLabel>
                  <Input
                    id="filtroRecebimentoAte"
                    type="date"
                    value={draft.recebimentoAte}
                    onChange={(e) =>
                      setDraftField("recebimentoAte", e.target.value)
                    }
                    className={dateFieldClass}
                  />
                </div>
          ...
          <DataTableCard>
            <DataTableToolbar
              ...
              reportTo="/relatorios/contas-receber"
            ...
            {loading ? (
              ...
              <DataTableEmptyState
                ...
                noResultsTitle="Nenhuma conta a receber encontrada com os filtros aplicados."
                createLabel="Cadastrar conta a receber"
                onCreate={() => navigate("/contas-receber/novo")}
                activeCount={activeCount}
                      ...
                      {paginated.map((i) => (
                        ...
                        >
                          <td className="whitespace-nowrap px-6 py-2.5">
                            <RowActionsDropdown
                              reportEndpoint={`/contas-receber/${i.id}/relatorio`}
                              reportFilename={`conta-receber-${i.id}.pdf`}
                              viewTo={`/contas-receber/${i.id}`}
                              editTo={`/contas-receber/${i.id}/editar`}
                              onDelete={() => setConfirmDelete(i)}
                            />
                          </td>
                          <td className="px-6 py-2.5">
                            <TableCellText text={i.nomeContaReceber} bold>
                              {i.nomeContaReceber}
                            </TableCellText>
                          </td>
                          <td className="px-6 py-2.5">
                            <TableCellText text={pagadorLabel(i)}>
                              {pagadorLabel(i)}
                            </TableCellText>
                          </td>
                          <td className="whitespace-nowrap px-6 py-2.5">
                            <StatusPill
                              status={i.classificacaoContaReceber}
                          ...
                          <td className="whitespace-nowrap px-6 py-2.5 text-right md:text-left">
                            <TableCellText
                              text={formatCurrency(parseMoney(i.valorAReceber))}
                              bold
                            >
                              {formatCurrency(parseMoney(i.valorAReceber))}
                            </TableCellText>
                          </td>
                          <td className="whitespace-nowrap px-6 py-2.5">
                            <StatusPill
                              status={i.formaPagamento}
                              ariaLabelPrefix="Forma de recebimento"
                            />
                          </td>
                          <td className="whitespace-nowrap px-6 py-2.5">
                            <StatusPill
                          ...
                          <td className="whitespace-nowrap px-6 py-2.5">
                            <GerarReciboButton
                              endpoint={`/contas-receber/${i.id}/recibo`}
                              filename={`recibo-recebimento-${i.id}.pdf`}
                              successMessage="Recibo de recebimento gerado com sucesso."
                              errorMessage="Não foi possível gerar o recibo de recebimento."
                            />
                          </td>
                        </tr>
                      ))}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {confirmDelete?.nomeContaReceber}”? Esta ação não pode ser
              desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
